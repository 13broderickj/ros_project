...
void projection_method3(pcl::PointCloud<pcl::PointXYZRGB>::Ptr  box_ptr, int npts_cloud, string fname);
...
float _mDis = 0.92*2;  // darkest, float!
float _nDis = 0.77*2; // brightest, float!
float _Nv = 120*3;
float _Nu = 120*3;
float _focal_len = 220.0; //220.0 may be good? float!
/*************************************/
// 500x250x250
...
int main(int argc, char** argv) {
    ...
    loadParameters(nh);
	{
		// Box Filter Parameters
		 ...
		 _bmZ =  atof(argv[6]);
		// Projection Parameters
		 _mDis =  atof(argv[7]);  // darkest, !
		 _nDis =  atof(argv[8]); // brightest, !
		 _Nv =  atof(argv[9]);
		 _Nu =  atof(argv[10]);
		 _focal_len =  atof(argv[11]); //220.0 may be good? !
		 _cX = atof(argv[12]); 
		_cY = atof(argv[13]); 
		_cZ = atof(argv[14]); 
