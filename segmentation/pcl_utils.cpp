...
//operates on transformed cloud PointCloud<pcl::PointXYZ>
//uses passthrough filter, which is MUCH faster!!
double PclUtils::find_table_height(double z_min, double z_max, double dz, PointCloud<pcl::PointXYZ>::Ptr &inputCloud) {
    vector<int> indices;
    pcl::PassThrough<pcl::PointXYZ> pass; //create a pass-through object    
    pass.setInputCloud(inputCloud); //set the cloud we want to operate on--pass via a pointer
    pass.setFilterFieldName("z"); // we will "filter" based on points that lie within some range of z-value
    double z_table = 0.0;
    int npts_max = 0;
    int npts;
