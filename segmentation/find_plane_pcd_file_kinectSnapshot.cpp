...
int main(int argc, char** argv) {
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr input_cloud(new pcl::PointCloud<pcl::PointXYZ>); //cloud to find table
    ...
    if (pcl::io::loadPCDFile<pcl::PointXYZ> (fname, *input_cloud) == -1) //* load the file
    {
        ROS_ERROR("Couldn't read file \n");
        return (-1);
    }
    ...
    pcl::io::savePCDFile("kinect_snapshot.pcd", *downsampled_kinect_ptr, true);
	pclUtils.find_table_height(-100,1000,0.001,input_cloud);
    return 0;
}
